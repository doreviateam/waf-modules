...
class PartnerAddress(models.Model):
    ...
    sale_line_dispatch_ids = fields.One2many(
        ...
        'partner_shipping_id',
        string='Dispatches',
        domain="[('partner_shipping_id.delivery_contact_id', '=', delivery_contact_id)]"
    )
    ...
    def create(self, vals_list):
        ...
        for record in records:
            # Créer un contact de livraison pour chaque partenaire
            for partner in record.partner_ids:
                delivery_contact = self.env['res.partner'].create({
                    ...
                    'zip': record.zip,
                    'type': 'delivery',
                    'parent_id': partner.id,  # Pour chaque partenaire
                })
                if partner == record.main_partner_id:
    ...
    def write(self, vals):
        ...
        if 'partner_ids' in vals:
            for record in self:
                ...
                # Supprimer les contacts des partenaires retirés
                for partner in old_partners - new_partners:
        ...
        if any(f in vals for f in ['name', 'street', 'city', 'zip']):
            for record in self:
                contacts_to_update = self.env['res.partner'].search([
                    ('parent_id', 'in', record.partner_ids.ids),
                    ('name', '=', record.name),
                    ('type', '=', 'delivery')
                ])
                contacts_to_update.write({
                    'name': record.name,
                    'street': record.street,
                    'city': record.city,
                    'zip': record.zip,
